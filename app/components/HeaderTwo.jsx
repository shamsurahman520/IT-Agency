...
const HeaderTwo = () => {
    ...
    useEffect(()=>{
        const handleScroll = ()=>{
            const elem = document.getElementById("service-heading")
            if(elem){
                const position = elem.getBoundingClientRect()
                setShowHeader(position.bottom< window.innerHeight )
            }
        }
    ...
    return (
      <>
      <header className={`fixed ${showHeader? "opacity-100": "-translate-y-[72px] opacity-0"} bg-white duration-500 shadow-md lg:shadow-xl md:shadow-xl px-5 py-4 z-30 w-full`}>
          ...
          <button
            ...
            ☰
              ...
              <ul className='md:flex hidden gap-8 items-center font-inter text-black cursor-pointer'>
                  ...
                  <li><Link href="#" className="relative text-[#F9004D] border-[1px] border-[#F9004D] 
                    w-32 h-12 py-2 rounded-3xl text-center block group"> 
                    Get Started
                   <div
                  className="absolute inset-0 mx-auto py-2 w-full bg-[#F9004D] group-hover:w-0 
